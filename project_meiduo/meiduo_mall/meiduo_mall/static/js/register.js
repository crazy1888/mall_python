    ...
    methods: { // 定义和实现事件方法
        ...
        send_sms_code(){
            ...
            axios.get(url,{
                ...
                .then(response=>{
                    if(response.data.code=='0'){    //从view获取的code
                        //展示60秒
                        let num=60;
                        let t = setInterval(()=>{
                            if(num == 1){   //倒计时即将结束
                                clearInterval(t);
                                this.sms_code_tip = '获取短信验证码';  //提示文字
                        ...
                        if(response.data.code=='4001'){ //图形验证码信息错误
                            this.error_image_code_message = response.data.errmsg;       //对应后端传过来的信息
